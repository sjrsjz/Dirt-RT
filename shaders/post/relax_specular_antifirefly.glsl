...
// Scalar outlier replacement would rotate MaxEntY without updating CoCg in
...
void main() {
    ...
    vec4 maxEntY = texelFetch(colortex9, ivec2(pixel), 0);
    imageStore(colorimg3, ivec2(pixel), maxEntY);

#if DEBUG_VIEW == 24
    RelaxPostSignal meta = relaxUnpackPost(
        texelFetch(colortex4, ivec2(pixel), 0));
    SpecularMaxEnt signal;
    signal.maxEntY = maxEntY;
