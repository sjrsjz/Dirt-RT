// Purpose: prepare reflection estimator variance and the independent-current branch for A-Trous.
// Dispatch: 16x16.
// Reads: colortex4 temporal proposal, colortex6 Raw RT observation, compact primary geometry.
// Writes: colortex3 geometry, colorimg5 prepared proposal, shared scratch plane A.
// Persistent side effects: prepared specular estimator-variance diagnostic only.
...
#if DEBUG_VIEW == DEBUG_VIEW_SPECULAR_PREPARED_ESTIMATOR_VARIANCE
    debugWriteSpecularPreparedEstimatorStandardDeviation(uvec2(pixel), estimatorStdDev);
#endif
}
